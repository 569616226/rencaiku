	...
	<body>
						...
						<div class="search-box-o flexbox">
							<div class="flexItem">
								<div class="search-input-box flexbox">
									<div class="flexItem">
										<input class="search-input pl30" type="text" placeholder="可输入姓名/手机号/职位" name="keyWord" id="keyWord">
									</div>
								</div>
							</div>
		...
		<script>
            ...
            * */
            ...
            var url = host + '/mobile/archive/search_all_data/'+'{{ $depart_id }}';
            ...
            $('#endTaskBtn').on('click', function() {
                ...
                let word = $('#word').val()
                window.location.href = '/mobile/archive/'+'{{ $depart_id }}'+'/search_all?offer_status='+status+'&offer_depart='+depart+'&keyWord='+encodeURI(encodeURI(word))
            });

            $('.search_letter_btn').on('click', function() {
                let keyWord = $('#keyWord').val()
                window.location.href = '/mobile/archive/'+'{{ $depart_id }}'+'/search_all?keyWord='+encodeURI(encodeURI(keyWord))
            });
		</script>
	</body>
